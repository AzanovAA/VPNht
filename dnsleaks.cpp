...
void DnsLeaks::enable(bool bEnabled)
{
    ...
    if (bEnabled)
    {
        qDebug() <<"------------------ DNS LEAK ON -------------";
        oldDnsLeak_.clear();
        assignNewDns("0.0.0.0,0.0.0.0", oldDnsLeak_);
        qDebug() << oldDnsLeak_;
        qDebug() <<"--------------------------------------------";
    }
    else
                    ...
                    || (ai->Type == IF_TYPE_IEEE80211))   // radio
            {
                            ...
                            &ulPerAdapterInfoSize)) == ERROR_SUCCESS)
                {
                    ...
                    if ( strstr(ai->Description, "TAP") == 0 )
                    {
                        ...
                        dns.insert( tmpadpif.AdapterName, tmpadpif.NameSever.c_str());
...
void DnsLeaks::restoreOldDns(QMap<QString, QString> &dns)
{
    QMap<QString, QString>::iterator i = dns.begin();
    while (i != dns.end()) {
        regSetDNS(i.key().toStdString().c_str(), i.value().toStdString().c_str());
