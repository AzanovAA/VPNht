...
#define SERVICE_PIPE_NAME  (L"\\\\.\\pipe\\VPNHtOpenVPNService")
...
int sendAll(SOCKET socket, char *buf, int size)
{
    ...
    while (!bStopThread_)
    {
        if (!bSockConnected_)
        {
            ...
            if (socketOpenVPN == INVALID_SOCKET)
            {
                ...
                {
                    qDebug() << "Can't connect to openvpn socket after 1000 retries";
                    currentState_ = STATUS_DISCONNECTED;
        ...
        if (bSockConnected_)
        {
            ...
            else if (strServerReply.contains(">BYTECOUNT:", Qt::CaseInsensitive))
            {
                ...
                if (pars.count() > 1)
                {
                    ...
                    if (pars2.count() == 2)
                    {
                        long l1 = pars2[0].toLong();
                        long l2 = pars2[1].toLong();
